...
if (isset($_REQUEST['trainer'])) {
  ...
  if (count($errors) == 0) {
    ...
    if ($sql) {
    $msg="<div class='alert alert-success'><b>Entraineur Ajouté</b></div>";
    }else{
      $msg="<div class='alert alert-warning'><b>Entraineur non Ajouté</b></div>";
    }
  }
}
...
<div class="container">
	<form class="mt-3 form-group" method="post" action="">
		<h3>AJOUT ENTRAINEUR</h3>
		 <?php include('errors.php'); 
    echo @$msg;

    ?>
		<label class="mt-3">ID ENTRAINEUR </label>
		<input type="text" name="id" class="form-control">
		<label class="mt-3">NOM ENTRAINEUR</label>
		<input type="text" name="name" class="form-control">
		<label class="mt-3">TEMPS</label>
		<input type="text" name="time" class="form-control">
		<label class="mt-3">NUM MOBILE</label>
		<input type="text" name="mobileno" class="form-control">
		<label class="mt-3">ID PAYEMENT</label>
		<input type="text" name="pay_id" class="form-control">
		<button class="btn btn-dark mt-3" type="submit" name="trainer">AJOUTER</button>
	</form>
</div>