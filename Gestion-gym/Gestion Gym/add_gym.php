...
if (isset($_REQUEST['gym'])) {
  ...
  if ($user) { 
    if ($user['gym_id'] === $gym_id) {
      array_push($errors, "<div class='alert alert-warning'><b>ID existant</b></div>");
    }
  }
  ...
  if (count($errors) == 0) {
    ...
    if ($sql) {
    $msg="<div class='alert alert-success'><b>Gym ajouté</b></div>";
    }else{
      $msg="<div class='alert alert-warning'><b>Gym non ajouté</b></div>";
    }
  }
}
...
<div class="w3-container">
    ...
    <option value="men">HOMMES</option>  
		...
		<button class="btn btn-dark mt-3" type="submit" name="gym">AJOUTER</button>
	</form>
</div>